...
class _FunctionCorrelation(torch.autograd.Function):
    ...
    def forward(self, one, two):
        rbot0 = one.new_zeros(
            [one.shape[0], one.shape[2] + 8, one.shape[3] + 8, one.shape[1]]
        )
        rbot1 = one.new_zeros(
            [one.shape[0], one.shape[2] + 8, one.shape[3] + 8, one.shape[1]]
        )
        ...
        if one.is_cuda == True:
            ...
            cupy_launch(
                "kernel_Correlation_rearrange",
                cupy_kernel(
                    "kernel_Correlation_rearrange", {"input": one, "output": rbot0}
                ),
            )(
            ...
            cupy_launch(
                "kernel_Correlation_rearrange",
                cupy_kernel(
                    "kernel_Correlation_rearrange", {"input": two, "output": rbot1}
                ),
            )(
