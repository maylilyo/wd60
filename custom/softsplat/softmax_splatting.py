...
def softmax_splatting(input: torch.Tensor, flow: torch.Tensor, metric: torch.Tensor):
    input = torch.cat([input * metric.exp(), metric.exp()], 1)

    tenOut = softsplat_func.apply(input, flow)
